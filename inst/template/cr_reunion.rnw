...
cejour = xl$CEJOUR %>% 
  ...
  mutate(ETAT = ifelse(is.na(ETAT), "af", ETAT)) # ETAT vide -> A faire
...
# supprime Fait/Validé/Annulé et supérieur à 3 semaines
taches = taches %>% 
  filter( !( (ETAT %in% c("f", "v", "an")) & (ECHEANCE <= date_reu-3*7) ) )
