...
# ordonner colonnes
ce_jour = ce_jour[, colnames(tache)]

# ajouter ce_jour à tache
tache = rbind(tache, ce_jour)

# ajouter rappel
row = (tolower(tache$ETAT) == "a") & (tache$ECHEANCE <= date_reu)
tache$PRIORITE[row] = "RAPPEL"

# ajouter urgent
row = (tolower(tache$ETAT) == "a") & (tache$ECHEANCE <= date_reu-sem)
tache$PRIORITE[row] = "URGENT"

# supprime Fait et supérieur à 3 semaines
row = !( (tolower(tache$ETAT) == "f") & (tache$ECHEANCE <= date_reu-3*sem) )
tache = tache[row,]
@
...
print_taches <- function(chapitre = "orga", col_format = "|p{11cm}|>{\\centering}p{2cm}|>{\\centering}p{2cm}|",
                         col_head = "lightgray") {
  cat("\\newpage\n")
  ...
  for (a in seq_along(acteurs)) {
    ...
    for (d in seq_along(dates)) {
      ...
      df_d$TACHE = paste0(rowformat, df_d$TACHE, "}")
      df_d$ECHEANCE = paste0(rowformat, df_d$ECHEANCE, "}")
      df_d$ETAT = paste0(rowformat, df_d$ETAT, "}")
