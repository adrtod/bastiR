...
source("cr_config.r", encoding = "ISO8859-1")
@
...
photo_files = list.files(backup, pattern = ".*\\.(jpg|jpeg|JPG|JPEG|png|PNG)", full.names = TRUE)
@
...
photo_files = resize_photos(photo_files = photo_files, folder = temp,
...
cejour = xl$CEJOUR %>% 
  filter(!is.na(SECTION)) %>%  # Efface les lignes vides
  mutate(ETAT = ifelse(is.na(ETAT), "af", ETAT)) # Complète `ETAT` manquant par `A faire`
...
# recodage et tri
taches = taches %>% 
  mutate(ETAT = tolower(ETAT)) %>% # Edite clés `ETAT`, `SECTION`, `ACTEUR` en minuscule
...
row = (taches$ETAT %in% c("af", "av")) & (taches$ECHEANCE <= date_reu)
taches$PRIORITE[row] = "RAPPEL"

row = (taches$ETAT %in% c("af", "av")) & (taches$ECHEANCE <= date_reu-7)
...
taches$DATEREALISATION[row] = date_reu

# Supprime les lignes `Fait`/`Validé`/`Annulé` avec `DATEREALISATION` supérieure à 3 semaines
taches = taches %>% 
  filter( !( (ETAT %in% c("f", "v", "an")) & (DATEREALISATION <= date_reu-3*7) ) )
@
...
xl_next$PHOTOS = xl$PHOTOS[NULL,] # NULL %%%%%%%%%%%%%%%% TEMP %%%%%%%%%%%%%
